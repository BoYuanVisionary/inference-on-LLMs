...
def MCTS_search(mcts_task):
    root = treeNode('')
    root.variance = mcts_task.root_variance
...
def executeRound(root, mcts_task):
    ...
    ##########################
    node.variance = node.parent.variance if node.variance is None else node.variance
    node.branch = 1 + int (node.variance * mcts_task.gamma)
    ##########################
...
def var_back_propagate(node):
    while node is not None:
        # using total variance to update the variance of the node
        child_vars = []
        child_vs = []
        for child in node.children.values():
        ...
        if len(child_vars) > 0:
            node.variance = numpy.mean(child_vars) + numpy.var(child_vs)
        node = node.parent
